...
class TestUserProfile(UserCase):
    def test_get_profile(self):
        ...
        for attr in expected_attrs:
            ...
            if attr == 'idcard':
                self.assertEqual(data[attr], misctools.mask_idcard_num(self.default_volunteer_attrs[attr]))
                continue

            if attr == 'role':
                self.assertEqual(data[attr], 'volunteer')
                continue

            if self.default_volunteer_attrs.get(attr) is not None:
                self.assertEqual(data[attr], self.default_volunteer_attrs[attr])
