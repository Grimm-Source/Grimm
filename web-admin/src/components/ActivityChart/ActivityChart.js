import { Radio, Progress, Alert } from "antd";
...
class ActivityChart extends React.Component {
  ...
  render() {
    ...
    const signUpRateDisabledPer = Number(
      ((signUpDisabled / targetDisabled) * 100).toFixed(2)
    );
    const signUpRateVolunteerPer = Number(
      ((signUpVolunteer / targetVolunteer) * 100).toFixed(2)
    );
    const signUpRateDisabledStr = `${signUpDisabled}/${targetDisabled}`;
    const signUpRateVolunteerStr = `${signUpVolunteer}/${targetVolunteer}`;

    const checkInRateDisabledPer = Number(
      ((checkInDisabled / signUpDisabled) * 100).toFixed(2)
    );
    const checkInRateVolunteerPer = Number(
      ((checkInVolunteer / signUpVolunteer) * 100).toFixed(2)
    );
    ...
    return (
      <div className="activity-chart">
        <div className="activity-chart-header">
          <div className="left-group">
            ...
            <div className="activity-progress">
              {this.state.chartType === CHART_TYPE.SIGN_UP ? (
                <div>
                  <span>
                    志愿者报名情况:{" "}
                    <span
                    ...
                    >
                      {signUpRateVolunteerStr}
                    </span>
                  </span>
                  <span>
                    <Progress
                      percent={signUpRateVolunteerPer}
                      status="active"
                    />
                  </span>
                  <span>
                    ...
                    <span
                      ...
                      onClick={this.props.onClickName}
                    >
                      {signUpRateDisabledStr}
                    </span>
                  </span>
                  <span>
                    <Progress percent={signUpRateDisabledPer} status="active" />
                  </span>
                </div>
              ) : (
                <div>
                  <span>
                    ...
                    >
                      {" "}
                      {checkInRateVolunteerStr}
                    </span>
                  </span>
                  <span>
                    <Progress
                      percent={checkInRateVolunteerPer}
                      status="active"
                    />
                  </span>
                  <span>
                    ...
                    >
                      {" "}
                      {checkInRateDisabledStr}
                    </span>
                  </span>
                  <span>
                    <Progress
                      percent={checkInRateDisabledPer}
                      status="active"
                    />
                  </span>
                </div>
              ...
              <Alert
                ...
                message={
                  this.state.chartType === CHART_TYPE.SIGN_UP ? (
                    <TextLoop mask>
                      <div>
                        {restSignUpVolunteer > 0
                          ? `志愿者还有${restSignUpVolunteer}个名额!`
                          : "正在加载..."}
                      </div>
                      <div>
                        {restSignUpDisabled > 0
                          ? `视障人士还有${restSignUpDisabled}个名额!`
                          : "正在加载..."}
                      </div>
                    </TextLoop>
                  ) : (
                    <TextLoop mask>
                      <div>
                        {restCheckInVolunteer > 0
                          ? `志愿者未签到${restCheckInVolunteer}人!`
                          : "正在加载..."}
                      </div>
                      <div>
                        {restCheckInDisabled > 0
                          ? `视障人士未签到${restCheckInDisabled}人!`
                          : "正在加载..."}
                      </div>
                    </TextLoop>
                  )
        ...
        <SignUpChart
          data={data}
          isSignUp={this.state.chartType === CHART_TYPE.SIGN_UP}
        />
      </div>
    );
  }
